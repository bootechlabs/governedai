...
- **RiskClassification** — id, ai_system_id (unique — current only, not versioned), use_case_template, answers (JSON), risk_tier, completed_by, completed_at. `triggered_regulations` (the old enum array) is deprecated/unused — see `RiskClassificationRegulation`.
...
Status as of 2026-09-15: slices 7, 9, and 10 shipped; slice 8 blocked on licensing (see below). All four scoped in one day.
...
**Slice 8 — CHAI risk-categorization alignment. Blocked on licensing (2026-09-15), not an engineering task as scoped.** Pulled the actual CHAI Risk Categorization Tool v3 PDF ([source](https://www.chai.org/blog/coalition-for-health-ai-chai-releases-version-3-of-its-risk-categorization)) rather than build from the blog post's summary: it has 19 real modifiers (9 Life & Patient Safety, 10 Technology & Data — e.g. Distance from Patient, Decision Autonomy, Use of Sensitive Data, AI Model Lifecycle Management), not "2-4 questions per domain." Its final page licenses it **CC BY-NC-ND 4.0** (Non-Commercial, No Derivatives — *"you may not distribute the modified material"*) and marks it draft/not-for-reliance. GovernedAI is commercial; reproducing CHAI's specific modifier content in a paid feature isn't something to build around quietly. Decision: don't build against this PDF. Path forward is either a licensing conversation with CHAI, or an independently-authored risk-categorization pair (own questions/definitions, not CHAI-branded) if the underlying idea is still wanted — not decided yet. Checked further (2026-09-15): the other CHAI resource considered as a possible alternative (governance.chai.org's "AI Governance Playbooks") carries the identical CC BY-NC-ND 4.0 license on its own license page — CHAI's standard terms apply across its published documents, not just the Risk Categorization Tool. No CHAI resource is currently usable as a basis for a commercial feature without CHAI's permission.
...
- Slice 8 (CHAI risk categorization) is blocked on licensing — whether to pursue a licensing conversation with CHAI or build an independently-authored replacement instead, not decided yet.
