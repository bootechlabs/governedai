...
- **Vendor** — id, organization_id, name, baa_status, subprocessors, soc2_report_url, model_card_url, security_eval_url, notes, last_attested_at, attestation_cadence_days, last_reattestation_notice_at.
...
**Slice 16 — scoped, not yet built:** report reframing around Baylor's three board-level litmus questions (inventory completeness, 24-hour action traceability, minutes-to-revoke) as the headline section for any agentic-flagged system's PDF/share-link report — a presentation layer over Slice 15's data, close to free once Slice 15 ships; audit-log integrity claims (a stated retention-policy line in the exported report, and hash-chaining `AuditLogEntry` rows if cheap enough) — turns "we log stuff" into "our logs are independently verifiable," a differentiator none of the six competitors claim; a small additive field set on `Vendor` for agent-platform posture (kill-switch API, permission-scoping documentation, audit-log export support), reinforcing the vendor-AI-first wedge.
