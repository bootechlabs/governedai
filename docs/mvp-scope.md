...
- **Inventory + dashboard**: `/systems` is a real dashboard (counts by deployment/risk tier, vendor BAA attention, pending workflow reviews, recent activity capped to the last 5) with a risk-tier donut and a hand-rolled bar chart of review status (derived per system from its workflow stages — not a stored field); the list itself lives at `/systems/inventory`.
...
- **Change events + recertification** (slice 6): a `ChangeEvent` per governance-relevant field edit (vendor, classification, deployment status, business unit, states deployed, agentic flag — one shared `keyChangeValues` helper feeds both the UI action and the API `PUT`, so a new field can't be missed in one path); recertification need is computed (not a stored flag) from whether a change is newer than the last stage decision, and re-opens the decision form on already-decided stages without resetting their status.
...
- **AiSystem** — id, organization_id, name, owner, business_unit, description, vendor_name (free text) + vendor_id (nullable link to Vendor), classification, deployment_status, states_deployed (string array, drives state-law regulation triggers), is_agentic (gates the agentic-governance question block), archived_at.
- **Vendor** — id, organization_id, name, baa_status, subprocessors, soc2_report_url, model_card_url, security_eval_url, agent_kill_switch_supported, agent_permission_scoping_documented, agent_audit_log_export_supported, notes, last_attested_at, attestation_cadence_days, last_reattestation_notice_at.
- **RiskClassification** — id, ai_system_id (unique — current only, not versioned), use_case_template, answers (JSON), risk_tier, life_safety_tier, tech_data_tier, agentic_risk_tier, completed_by, completed_at. `triggered_regulations` (the old enum array) is deprecated/unused — see `RiskClassificationRegulation`.
...
- **AuditLogEntry** — id, ai_system_id, actor_id, action, detail (JSON), occurred_at, previous_hash, hash (sequentially hash-chained per AiSystem — see `src/lib/audit-log.ts`).
...
**Slice 14 — Adversarial/red-team security evaluation as a first-class evidence type.** Shipped (2026-09-16). Three small, additive pieces, all extending existing machinery rather than crossing into the excluded runtime-enforcement/red-teaming-suite lanes:
...
## Differentiation roadmap — v1.5 (agentic AI governance research, added 2026-09-17)

Full analysis: `agentic-ai-governance-research-2026-09-17.md` — a named, credible source for once (Baylor University's Hankamer School of Business, CyBear Essentials series), not an uncredited graphic, corroborated by a cluster of Medium pieces (ODSC's healthcare-agent blueprint, Singapore's IMDA framework, UC Berkeley's Agentic Risk-Management Standards Profile, an AI-agent audit checklist). Throughline: static, model-level governance (classify once, attach evidence, route for approval — GovernedAI's MVP) is necessary but insufficient once a system is agentic — continuous, re-visitable control matters, not a one-time deployment checkbox. Critically, most of what's asked for is *documentation that a control exists* (a named owner, a tested revocation procedure, a logging policy), which keeps this inside GovernedAI's audit/evidence lane rather than the Fiddler/Zenity/Darktrace runtime-enforcement lane already excluded.

**Slice 15 — Agentic AI System assessment, built on Baylor's five governance dimensions (Identity, Behavior, Data Boundaries, Access Scope, Failure Response).** Shipped (2026-09-17), highest-priority item from this research. `AiSystem` gains `isAgentic` (edit-form checkbox, same pattern as `statesDeployed`); when set, the risk-assessment questionnaire gains a new gated supplemental question block (5 questions, one per Baylor dimension) scored into a new `RiskClassification.agenticRiskTier` via the same weighted-scoring mechanism as the existing `lifeSafetyTier`/`techDataTier` — but citing Baylor by name in the UI/PDF disclaimer, unlike those two, which are explicitly original. Note: Baylor's Identity dimension ("named accountable owner") is largely already satisfied by `AiSystem.owner`, which has always been a required linked `User`, not free text as the source research assumed — the new Identity question instead asks whether the *agent itself* has a distinct, traceable identity/credential, a genuinely separate fact. Also adds two evidence categories from the same research (`OVERRIDE_REVOCATION_PROCEDURE`, `ACCESS_LIFECYCLE_POLICY`), batched in since they're trivial additions to the same enum touched for evidence in slice 14.

**Slice 16 — Shipped (2026-09-17).** Report reframing around Baylor's three board-level litmus questions as a PDF headline section for any agentic-flagged system (derived from Slice 15's assessment answers, no new data); audit-log integrity — `AuditLogEntry` is now sequentially hash-chained per `AiSystem` (`previousHash`/`hash`, sha256), with `verifyAuditChain` making "tamper-evident" a checkable claim rather than a stated line, surfaced on the Audit tab and in the PDF's Audit Trail section; `prisma/backfill-audit-hashes.ts` retroactively chains pre-existing rows (idempotent — **must be run once against production** after this deploys, same as any prior backfill script). A small additive field set on `Vendor` for agent-platform posture (kill-switch API, permission-scoping documentation, audit-log export support), reinforcing the vendor-AI-first wedge.

**Correction, not new scope:** the research's recommendation to "raise the priority of Change Event detection (slice 6) and vendor re-attestation (slice 10)" is moot — both already shipped (see "Shipped" above); the research was reasoning from a stale/generic snapshot of the product, not this codebase's actual state.

**Messaging note, not a build item:** Baylor's "governance as business assurance, not a brake on speed" framing and the audit guide's "paper governance vs. retrofitted evidence vs. static compliance" distinction are strong, board-credible language for whenever the paused LinkedIn content motion resumes — same treatment as the MIT FutureTech stat in the prior gap-analysis doc (cite Baylor directly, don't reuse uncited).

**Caveat carried forward:** the audit-guide piece's adoption statistics (72% using/planning agentic AI, 72% of S&P 500 disclosing material AI risk with only 26% having comprehensive governance) have no sourcing in the original article — same bar as the MIT stat check: find and cite the primary study before reusing, or don't use them.
